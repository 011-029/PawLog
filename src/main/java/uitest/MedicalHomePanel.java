...
public class MedicalHomePanel extends JPanel {
    ...
    private JComponent createContent(List<MedicalRecord> medical) {
        ...
        PlaceholderTextField searchField =
                new PlaceholderTextField("통합 검색 (증상/병원/약 이름 등)");
        searchField.setPreferredSize(new Dimension(310, 40));
        searchField.setMaximumSize(new Dimension(Integer.MAX_VALUE, 40));   // ★ 수정
        searchField.putClientProperty("FlatLaf.style", "arc:10");
        searchField.setAlignmentX(Component.LEFT_ALIGNMENT);                // ★ 수정
        ...
        JPanel btn1 = createMenuButton("진료 기록", "icons/medical.svg",
                () -> mainFrame.switchPanel(
                        new MedicalRecordListPanel(mainFrame, MedicalMgr.getInstance().mList)));
        ...
        JPanel btn3 = createMenuButton("예방접종 기록", "icons/syringe.svg",
                () -> mainFrame.switchPanel(
                        new VaccineRecordListPanel(mainFrame, VaccineMgr.getInstance().mList)));
